...
struct ifList* ifHead = NULL;

/**
 * Write branch to else label based on conditionn test
 *
 * @param instList instructions
 * @param reg condition register check
 * @param symtab a symbol table
 */
void emitIfStart(DList instList, int reg, SymTable symtab) {
	char* regName = (char*)SymGetFieldByIndex(symtab, reg, SYM_NAME_FIELD);

	char s[10];
	sprintf(s, "%d", ifCount);

	char* inst = nssave(5, "\tbeq ", regName, ", $0, ", "else", s);
	dlinkAppend(instList,dlinkNodeAlloc(inst));
	
	struct ifList* newIf = (struct ifList*)malloc(sizeof(struct ifList));
	newIf->idx = ifCount;
	newIf->next = ifHead;	
	ifHead = newIf;
	ifCount++;
...
/**
 * Write end label for if statementF
 *
 * @param instList instructions
 */
void emitIfEnd(DList instList) {
	if (ifHead == NULL) return;
	char s[10];
	sprintf(s, "%d", ifHead->idx);
	char* inst = nssave(3, "endif", s, ":\tnop");
	dlinkAppend(instList,dlinkNodeAlloc(inst));

	struct ifList* tempIf = ifHead;
	ifHead = ifHead->next;
	free(tempIf);
}

/**
 * Write else label and branch to end label for if the if condition was true
 *
 * @param instList instructions
 */
void emitElseStart(DList instList) {
	if (ifHead == NULL) return;
	char s[10];
	sprintf(s, "%d", ifHead->idx);
	char* inst = nssave(3, "\tb ", "endif", s);
	dlinkAppend(instList,dlinkNodeAlloc(inst));

	char* inst2 = nssave(3, "else", s, ":\tnop");
...
struct LoopList* loopHead = NULL;

void emitLoopStart(DList instList) {
	char s[10];
	sprintf(s, "%d", loopCount);
	char* inst = nssave(3, "startLoop", s, ":\tnop");
	dlinkAppend(instList,dlinkNodeAlloc(inst));
	
	struct LoopList* newLoop = (struct LoopList*)malloc(sizeof(struct LoopList));
	newLoop->idx = loopCount;
	newLoop->next = loopHead;	
	loopHead = newLoop;
	loopCount++;
}

void emitLoopCheck(DList instList, int reg, SymTable symtab) {
	char* regName = (char*)SymGetFieldByIndex(symtab, reg, SYM_NAME_FIELD);
	char s[10];
	if (loopHead == NULL) return;
	sprintf(s, "%d", loopHead->idx);

	char* inst2 = nssave(4, "\tbeq ", regName, ", $0, endLoop", s);
	dlinkAppend(instList,dlinkNodeAlloc(inst2));
}

/**
 * Write end label for loop
 *
 * @param instList instructions
 */
void emitLoopEnd(DList instList) {
	if (loopHead == NULL) return;
	char s[10];
	sprintf(s, "%d", loopHead->idx);
	char* inst = nssave(2, "\tb startLoop", s);
	dlinkAppend(instList,dlinkNodeAlloc(inst));

	char* inst2 = nssave(3, "endLoop", s, ":\tnop");
	dlinkAppend(instList,dlinkNodeAlloc(inst2));

	struct LoopList* tempLoop = loopHead;
...
void createArray(DList dataList, char* name, int size) {
	char amt[20];
	sprintf(amt, "%d", (size*4));
	char* inst = nssave(1, "\t.align 2");
	dlinkAppend(dataList,dlinkNodeAlloc(inst));
	char* inst2 = nssave(4, "array", name, ": .space ", amt);
	dlinkAppend(dataList,dlinkNodeAlloc(inst2));
}

/**
 * Get the address of an array value
 *
 * @param instList instructions
 * @param symtab symbol table
 * @param aName array name
 * @param idxReg index register
 * @return register index of address value
 */
int getArrayValueAddress(DList instList, SymTable symtab, char* aName, int idxReg) {
	int regIndex = getFreeIntegerRegisterIndex(symtab);
	char* regName = (char*)SymGetFieldByIndex(symtab,regIndex,SYM_NAME_FIELD);
	char* idxName = (char*)SymGetFieldByIndex(symtab,idxReg,SYM_NAME_FIELD);
	char* inst = nssave(4, "\tla ", regName, ", array", aName);
	dlinkAppend(instList,dlinkNodeAlloc(inst));
	int tempIdx = getFreeIntegerRegisterIndex(symtab);
	char* tempName = (char*)SymGetFieldByIndex(symtab,tempIdx,SYM_NAME_FIELD);
	char* inst2 = nssave(5, "\tsll ", tempName, ", ", idxName, ", 2");
	dlinkAppend(instList,dlinkNodeAlloc(inst2));
	char* inst3 = nssave(6, "\tadd ", regName, ", ", regName, ", ", tempName);
	dlinkAppend(instList,dlinkNodeAlloc(inst3));
	freeIntegerRegister((int)SymGetFieldByIndex(symtab,idxReg,SYMTAB_REGISTER_INDEX_FIELD));
	return regIndex;
}